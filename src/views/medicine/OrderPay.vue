...
onMounted(() => getOrderPre())
onMounted(() => getAddress())

// 3.创建药品订单
const show = ref(false) // 控制遮罩层
...
const openPay = async () => {
  ...
  if (!orderPre.value?.id) return showFailToast('不是有效的处方id')
  // 获取订单id
  if (!orderId.value) {
    loading.value = true
    try {
...
<template>
  <div class="order-pay-page" v-if="orderPre && address">
    ...
    ></van-submit-bar>
    <cp-pay-sheet
      pay-callback="http://localhost:5173/medicine/pay/result"
      v-model:show="show"
      :order-id="orderId"
