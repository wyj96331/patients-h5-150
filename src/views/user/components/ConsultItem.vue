...
import { cancelOrder, deleteOrder } from '@/api/onsult'
...
const onSelect = (action: { text: string }, i: number) => {
  ...
  // 查看处方
  if (i === 0) lookPre(props.item.prescriptionId)
}
...
<template>
  <div class="consult-item">
    ...
    <div class="foot" v-if="item.status === OrderType.ConsultPay">
      <van-button
        ...
        >取消问诊</van-button
      >
      <van-button @click="show = true" ick type="primary" plain size="small" round>
        去支付
      </van-button>
    </div>
    <!-- 2. 待接诊：取消问诊+继续沟通 -->
  ...
  <!--    支付组件-->
  <cp-pay-sheet v-model:show="show" :order-id="item.id" :actualPayment="item.payment" />
</template>
